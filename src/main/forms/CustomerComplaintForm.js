...
export default class CustomerComplaintForm extends React.Component {
	...
	submit = () => {
		const {userId, email, name, subject, type, message, customerId, customerName} = this.state;

		if (userId == '' || email == '' || name == '' || subject == '' || message == '' || customerId == '') {
	...
	render() {
		const {navigation} = this.props;
		const {userId, subject, name, message, customerId, customerName, isLoading, isPopup} = this.state;
		return (
			<WrapperMain>
