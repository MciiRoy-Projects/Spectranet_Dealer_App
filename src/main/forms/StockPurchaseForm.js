...
import {Header, WrapperMain, H1, Button, HeaderBack, H2} from '../../partials/_components';
import AppColors from '../../lib/_colors';
import AppIcons from '../../partials/_icons';
import {RF, RW, RH} from '../../lib/_sizes';
import {getData, idCheck, requestForm, keyContact, Snack} from '../../partials/_api';
...
export default class StockPurchaseForm extends React.Component {
	...
	submit = () => {
		const {userId, email, name, subject, type, message, product, quantity, tsm, tsmemail, superdealemail} = this.state;

		if (userId == '' || email == '' || name == '' || subject == '' || message == '' || type == '') {
	...
	render() {
		...
		const {userId, subject, name, message, product, quantity, tsm, isLoadingBg, isLoading} = this.state;
		return (
			<WrapperMain>
				...
				<View style={styles.paneTwo}>
					...
					<ScrollView showsVerticalScrollIndicator={false}>
						...
						<TextInput value={tsm} style={styles.input2} placeholder="tsm" editable={false} />
						<TextInput style={styles.input} placeholder="Product" value={product} onChangeText={product => this.setState({product})} />
						<TextInput
							style={styles.input}
						...
						/>
						<TextInput style={styles.input} placeholder="Subject" value={subject} onChangeText={subject => this.setState({subject})} />
						<TextInput
